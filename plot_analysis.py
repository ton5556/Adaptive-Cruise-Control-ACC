...
Usage:
    python3 plot_analysis.py
        → ใช้ไฟล์ล่าสุดใน LOG_DIR

    python3 plot_analysis.py acc_x.csv lead_x.csv
        → ระบุไฟล์เอง
"""
...
# Windows:
LOG_DIR = r"D:/Arduino/Adaptive Cruise Control ACC/acc_logs"
...
# =========================================================
df_acc['t'] = (
    df_acc['time_ms'] - df_acc['time_ms'].iloc[0]
) / 1000.0

df_lead['t'] = (
    df_lead['time_ms'] - df_lead['time_ms'].iloc[0]
) / 1000.0
...
dist_err = (
    df_acc['dist_mm'] - TARGET_DIST
).abs().mean()

print("\n===== ACC Performance =====")
print(f"Distance mean  : {dist_mean:.1f} mm")
print(f"Distance std   : {dist_std:.1f} mm")
print(f"Mean abs error : {dist_err:.1f} mm")

print(
    f"Speed range    : "
    f"{df_acc['speed_avg'].min():.0f}"
    f" – "
    f"{df_acc['speed_avg'].max():.0f} mm/s"
)
...
err = df_acc['dist_mm'] - TARGET_DIST
...
# SAVE FIGURE
# =========================================================
acc_dir  = os.path.dirname(acc_file)
acc_name = os.path.basename(acc_file)

out_name = (
    acc_name
    .replace("acc_", "analysis_")
    .replace(".csv", ".png")
)

out_path = os.path.join(acc_dir, out_name)
